...
- 临时 wrapper 必须把最终退出码写入 sidecar `.exitcode` 文件，并用同一个退出码退出；GUI 优先读取 sidecar，避免 WinForms/PowerShell 进程对象偶发显示空 `ExitCode`。
- wrapper 的 `catch` 必须先设置失败退出码，再用非终止方式输出错误，避免 `$ErrorActionPreference = 'Stop'` 让错误处理本身中断并误写成功。
