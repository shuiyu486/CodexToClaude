...
- Watchdog 只处理启动后的新 `/v1/messages` 长请求；默认 60 秒仍未完成时重启当前 CLIProxy provider。
...
| 改动 | 额外验证 |
|---|---|
| 代理逻辑 | `Auto host:port -> http://...`、`Socks5 -> socks5://...`、`Direct` 省略代理并清理 CLIProxy auth `proxy_url` |
| GUI | UI 脚本可解析、`CodexToClaude-GUI.cmd` 存在、中英文切换、快速向导关键路径 |
